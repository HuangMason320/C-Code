...
int Rarray[256] = {0}, LoadCheck[3] = {0, 0, 0};
...
void codegen(AST *root)
{
    ...
    if (root != NULL)
    {
        ...
        case ASSIGN: //"="
            ...
            if (right->kind == LPAR)
            {
                while (right->kind == LPAR)
                    right = right->mid;
                codegen(right);
            }
            else
                codegen(right);
            printf("store [%d] r%d\n", (left->Rnum) * 4, right->Rnum);
            LoadCheck[left->Rnum] = 0;
        ...
        case ADD: //"+"
            ...
            codegen(right);
            printf("add r%d r%d r%d\n", left->Rnum, left->Rnum, right->Rnum);
            root->Rnum = left->Rnum;
            Rarray[right->Rnum] = 0;
            break;
        case SUB: //"-"
        ...
        case PREINC: //"++"
            ...
            if (right->kind == LPAR)
            {
                ...
                codegen(right);
            printf("add r%d r%d 1\n", right->Rnum, right->Rnum);
            printf("store [%d] r%d\n", (right->Rnum) * 4, right->Rnum);
            root->Rnum = right->Rnum;
            break;
        case PREDEC: //"--"
            right = root->mid;
